...
from modules.styling import PREMIUM_CSS, section, kpi_card, journal_html
...
# ══════════════════════════════════════════════════════════════════════════════
with t6:
    st.markdown(section("⑥", "Subsequent Period Valuation"), unsafe_allow_html=True)

    subseq_type = st.selectbox(
        "Settlement Type for Subsequent Period",
        ["Equity-settled", "Cash-settled", "Employee choice (cash or equity)"],
    ...
    if subseq_type == "Equity-settled":
        st.info("**Equity-settled:** Fair value is fixed at grant date. Only the expected number of vested instruments is updated.")
        c1, c2 = st.columns(2)
        with c1:
        ...
        st.markdown(section("📊", "Equity-Settled – Subsequent Period Results"), unsafe_allow_html=True)
...
Current period expense:      {fc(subseq_result['current_period_expense'])}  (catch-up adjustment)</div>""", unsafe_allow_html=True)
        ...
        st.markdown(section("📒", "Journal Entry – Current Period"), unsafe_allow_html=True)
        st.markdown(journal_html(subseq_result["entries"]), unsafe_allow_html=True)

    # ── CASH-SETTLED ─────────────────────────────────────────────────────────
    elif subseq_type == "Cash-settled":
        st.info("**Cash-settled:** Liability is remeasured at current fair value. Full Black-Scholes recalculation at reporting date.")
        c1, c2 = st.columns(2)
        with c1:
        ...
        try:
            ...
            st.markdown(section("📊", "Cash-Settled – Subsequent Period Results"), unsafe_allow_html=True)
            pl_sign = "green" if subseq_result["pl_impact"] >= 0 else "red"
            khc = '<div class="kpi-grid">'
            khc += kpi_card("Current FV / Option", fc(subseq_result["current_fv_per_option"]), "Black-Scholes at reporting date", "blue" if True else "")
...
P&L impact:                  Closing - Opening + Cash = {fc(subseq_result['pl_impact'])}</div>""", unsafe_allow_html=True)
            ...
            st.markdown(section("📒", "Journal Entries – Current Period"), unsafe_allow_html=True)
            st.markdown(journal_html(subseq_result["entries"]), unsafe_allow_html=True)
    ...
    # ── EMPLOYEE CHOICE ──────────────────────────────────────────────────────
        ...
        st.markdown(section("📊", "Employee Choice – Subsequent Results"), unsafe_allow_html=True)
